...
# Create helper columns in the data.frame for comparing log distributions
...
grid.arrange(ggplot(aes(x=Price.EUR), 
        data = car_data) +
  geom_histogram(bins = 30) ,
   ggplot(aes(x=1, y=Price.EUR), 
        data = car_data) +
  geom_boxplot( )  , nrow =1)
```
...
```{r, echo=FALSE}
summary(car_data$Mileage.km)
```

The shape of the distribution (excluding damaged cars) is fairly symmetrical, 
except the huge bar around 0. 
...
ggplot(undamaged_used_cars, aes(Age.year)) +
  geom_histogram(binwidth = 1, 
                 aes(y=..density..), colour='grey', fill='white') +
  stat_function(fun=dnorm, 
...
```{r, echo=FALSE}
summary(undamaged_used_cars$Age.year)
```
...
```{r, echo=FALSE}
summary(undamaged_used_cars$Mileage.year)
```
...
higher number of possible variations of the model might create a diverse 
dataset. Many of the categorical columns (eg. color, transmission type, fuel, 
etc.) are heavily concentrated around one or few typical values, though 'exotic' 
variants are also present in the data.
...
the most common transformation do not provide meaningful benefits in these 
cases.
...
We have some strong positive and negative correlations besides the age-price 
relationship. 

```{r, echo=FALSE, error=FALSE, warning=FALSE, message=FALSE}
ggcorr(undamaged_used_cars, label=T, label_size = 3, 
       label_round = 2, label_alpha = T, hjust = 1, size=3)
```
...
ggpairs(undamaged_used_cars, columns = c('Price.log', 'Mileage.km', 
        ...
        lower = list(continuous = wrap("points", alpha = 0.1, size=1)))
```
...
better, but the relationship is still not very strong in this form, the
correlation coefficient is slightly higher than -0.6.
...
```{r, echo=FALSE}
cor.test(undamaged_used_cars$Mileage.km, undamaged_used_cars$Price.EUR)
```
...
```{r, echo=FALSE, }
cor.test(undamaged_used_cars$Age.year, log10(undamaged_used_cars$Price.EUR))
```
...
```{r, echo=FALSE}
cor.test(log10(undamaged_used_cars$Mileage.year), log10(undamaged_used_cars$Price.EUR))
```
...
with a few outliers (138 cm3 is clearly not true, while 350hp might be the 
result of some tuning).
...
```{r, echo=F}
ggplot(undamaged_used_cars, aes(cut(undamaged_used_cars$Capacity,
                            c(900, 1200, 1500, 1700, 2000, 2500, 3000)), Horsepower)) + 
  geom_boxplot(alpha=0.4)+
      theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  xlab('Capacity categories')
```

2D bins help to avoid overplotting and also nicely packs the slightly different
engine capacity values into bins. There's clearly a positive relationship 
between engince capacity and the power (not surprisingly).
...
```{r, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE}
ggplot(subset(undamaged_used_cars, Form %in% major_forms), 
       aes(y=Age.year, x=Form, color=Form, fill=Form)) +
...
seem to be a very strong one, especially when we filter out the damaged or 
unused cars from the sample. Most likely age is the major factor in the pricing 
of the cars.
...
Colors do not seem to have a huge impact on age and price distribution based on
this facet grid.
...
I prefer this plot over using boxplot in this particular case because boxplot 
partially masks the shape of the underlying distribution.
...
grouped around they median and mean (the distribution is pretty symmetrical 
too).
...
ggplot(subset(undamaged_used_cars, 
  ...
  facet_grid(Form~factor(Paint.group, 
                         levels = c('Common', 'Regular', 'Exotic'))) +
  theme(legend.position = 'none') +
  geom_smooth(method=lm, se=F, color='gray', linetype='dashed', size=0.5) +
  xlab('Age (years)') +
  ylab('Price (EUR) - log scale') +
...
price the seller asks, neither if the car has one of the most frequent colors 
nor if it's one of the exotic variants. All the plots show very similar 
distribution, except where we have too few observation, and exepct the minivans, 
that we already saw somewhat differs from the other categories.
